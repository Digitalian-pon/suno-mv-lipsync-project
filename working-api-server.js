...
// HTML content embedded in server
const HTML_CONTENT = `<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>🎵 Working API - Suno Creator</title>
    <style>
...
<body>
    <div class="container">
        <h1>🎵 Working API - Suno Creator</h1>
        
        <div class="connection-info">
            <strong>✅ サーバー接続成功</strong><br>
            プロキシサーバー経由でGemini APIに接続します<br>
            CORS制限を完全に回避できます
        </div>
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (result.error.includes('API_KEY_INVALID')) {
                        updateStatus('❌ API Keyが無効です。正しいKeyを入力してください', 'error');
                    } else if (result.error.includes('QUOTA_EXCEEDED')) {
                        updateStatus('⚠️ APIクォータを超過しています。時間をおいて再試行してください', 'warning');
                    }
                }
            } catch (error) {
...
const server = http.createServer((req, res) => {
    ...
    // API test endpoint
    if (req.url === '/test-api' && req.method === 'POST') {
        let body = '';
    ...
    if (req.url === '/generate-music' && req.method === 'POST') {
        let body = '';
        req.on('data', chunk => body += chunk);
        req.on('end', async () => {
            try {
